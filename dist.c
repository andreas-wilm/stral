...
void drawline(long i, double scale, node *start, boolean rooted)
{
  ...
  if ((long)p->ycoord == i && p->tip) {
    int strln;
	 ...
	 strln = (int)p->index - 1;
	 if (strln < spp *2 -1) {
	 fprintf(outfile, "%d ", (int)p->index);
	 for (j = 0; j < strlen(nayme[strln]); j++)
...
void treeoutr(node *p, long *col, tree *curtree)
{
  ...
  if (p->tip) {
    n = 0;
    for (i = 1; i <= strlen(nayme[p->index - 1]); i++) {
