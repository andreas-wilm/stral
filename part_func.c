...
/*
  ...
  Revision 1.8  2005/10/18 11:19:00  wilm
  autotools added

  Revision 1.7  2005/10/13 15:57:49  dalli
  major update: STRAL VERSION 3.0.1

  Revision 1.6  2005/07/08 07:48:00  dalli
  - original sequence strings in aln-output

  Revision 1.5  2005/07/07 14:31:41  dalli
  - seq ToACGUN

  Revision 1.4  2005/07/04 11:08:31  dalli
  read from prob-files

  Revision 1.3  2005/07/01 10:43:34  dalli
  minor updates

  Revision 1.2  2005/06/27 12:11:09  dalli
  weighting sequences / command line args updated---------------------------

  Revision 1.1.1.1  2005/06/06 16:00:03  dalli
  Initial Import
...
PUBLIC  float pf_fold(char *sequence, char *structure, int seqID, int nseqs);
...
PRIVATE void mysprintf_bppm(int length, char *sequence, int seqID, int nseqs);	/* GST */
void print_data_content1(struct vector *current_ptr);
void print_data_content(struct vector *current_ptr);
...
PUBLIC float pf_fold(char *sequence, char *structure, int seqID, int nseqs)
{
  ...
  for (j = TURN + 2; j <= n; j++)
      ...
      for (i = j - TURN - 1; i >= 1; i--)
	{
	  ...
	  if (type != 0)
		...
		qbt1 =
		  expHairpinEnergy (u, type, S1[i + 1], S1[j - 1],
				    sequence + i - 1);

	      /* interior loops with interior pair k,l */
	      for (k = i + 1; k <= MIN1 (i + MAXLOOP + 1, j - TURN - 2); k++)
		{
		  u1 = k - i - 1;
		  for (l = MAX1 (k + TURN + 1, j - 1 - MAXLOOP + u1); l < j;
		       l++)
		    {
	  ...
	  if (qm1)
	    qm1[jindx[j] + i] = qqm[i];	/* for stochastic backtracking */
  ...
  free_energy =
    (-log (Q) - n * log (pf_scale)) * (temperature + K0) * GASCONST / 1000.0;
  ...
  if (do_backtrack)
      ...
      for (l = n; l > TURN + 1; l--)
	{
	  ...
	  for (k = 1; k < l - TURN; k++)
		...
		continue;
	      ...
	      for (i = MAX1 (1, k - MAXLOOP - 1); i <= k - 1; i++)
		for (j = l + 1; j <= MIN1 (l + MAXLOOP - k + i + 2, n); j++)
		  {
		    ij = iindx[i] - j;
		    type = ptype[ij];
		    if ((pr[ij] > 0))
		      {
			pr[kl] +=
			  pr[ij] * expLoopEnergy (k - i - 1, j - l - 1, type,
						  type_2, S1[i + 1],
						  S1[j - 1], S1[k - 1],
						  S1[l + 1]);
		      }
	...
	for (j = i + TURN + 1; j <= n; j++)
      ...
      mysprintf_bppm (n, sequence, seqID, nseqs );	/* GST */
    }				/* end if (do_backtrack) */
...
scale_pf_params (unsigned int length)
{
  ...
  for (i = 0; i <= MIN1 (30, length); i++)
    {
      GT = hairpin37[i] * TT;
      exphairpin[i] = exp (-GT * 10. / kT);
    }
  for (i = 0; i <= MIN1 (30, MAXLOOP); i++)
    {
      GT = bulge37[i] * TT;
  ...
  for (i = 0; i < 5; i++)
    {
      GT = F_ninio37[i] * TT;
      for (j = 0; j <= MAXLOOP; j++)
	expninio[i][j] = exp (-MIN1 (MAX_NINIO, j * GT) * 10 / kT);
    }
  ...
  for (i = 0; i <= NBPAIRS; i++)
	...
	{
	  GT =
	    mism_H[i][j][k] - (mism_H[i][j][k] - mismatchI37[i][j][k]) * TT;
	  expmismatchI[i][j][k] = exp (-GT * 10.0 / kT);
	  GT =
	    mism_H[i][j][k] - (mism_H[i][j][k] - mismatchH37[i][j][k]) * TT;
	  expmismatchH[i][j][k] = exp (-GT * 10.0 / kT);
	  GT =
	    mism_H[i][j][k] - (mism_H[i][j][k] - mismatchM37[i][j][k]) * TT;
...
/*----------------------------------------------------------------------*/
PRIVATE double
expHairpinEnergy (int u, int type, short si1, short sj1, const char *string)
{
  double q;
  q = exphairpin[u];
  if ((tetra_loop) && (u == 4))
  ...
  if ((u1 == 0) && (u2 == 0))	/* stack */
	...
	{			/* interior loop */
	  if (u1 + u2 == 2)	/* size 2 is special */
	    ...
	    {
	      z = expinternal[u1 + u2] *
		expmismatchI[type][si1][sj1] * expmismatchI[type2][sq1][sp1];
...
PRIVATE void
get_arrays (unsigned int length)
{
  unsigned int size, i;
...
#define L 3
PRIVATE void
sprintf_bppm (int length, char *structure)
{
  int i, j;
...
/* GST >>> */
PRIVATE void
mysprintf_bppm (int length, char *sequence, int seqID, int nseqs)
{
	
  int i, j;
  float P[L * length];		/* P[][0] unpaired, P[][1] upstream p, P[][2] downstream p */
  
 
	
	last_ptr[seqID] = first_ptr[seqID] = NULL;
  for (j = 1; j <= length; j++)
    {
      P[j - 1] = 1.0;
      P[j - 1 + 1 * length] = P[j - 1 + 2 * length] = 0.0;
      for (i = 1; i < j; i++)
	{
	  P[j - 1 + 2 * length] += pr[iindx[i] - j];	/* j is paired downstream */
	  P[j - 1] -= pr[iindx[i] - j];	/* j is unpaired */
	}
      for (i = j + 1; i <= length; i++)
	{
	  P[j - 1 + length] += pr[iindx[j] - i];	/* j is paired upstream */
	  P[j - 1] -= pr[iindx[j] - i];	/* j is unpaired */
	}
    }
  ...
  for (j = 0; j < length; j++){
      char b = sequence[j];
		char ori = sdata[seqID].orisequence[j];
      int rb = -1;
      ...
      new_item_ptr = malloc (sizeof (struct vector));
		...
		i = 0;	
		new_item_ptr->seqID = seqID;
		new_item_ptr->base = b;
		new_item_ptr->oribase = ori;
		new_item_ptr->pos = j+1;
		new_item_ptr->oripos = j+1;
		new_item_ptr->replbase = rb;	
		new_item_ptr->p0 = P[j + i * length];


		i = 1;	
		new_item_ptr->p1 = P[j + i * length];

		i = 2;
		new_item_ptr->p2 = P[j + i * length];	

		new_item_ptr->next_ptr = first_ptr[seqID];
		if (last_ptr[seqID] == NULL)	{
			last_ptr[seqID] = new_item_ptr;
			last_ptr[seqID]->previous_ptr = NULL;
		} else {
			new_item_ptr->next_ptr->previous_ptr = new_item_ptr;
		new_item_ptr->previous_ptr = NULL;
		}
		first_ptr[seqID] = new_item_ptr;

	 
	}
#ifdef DEBUG	
	if (!verbose){
		print_data_content(last_ptr[seqID]);
		printf ("\n");
		print_data_content1(last_ptr[seqID]);
  		printf ("\n");
	}
#endif
}
...
pbacktrack (char *seq)
{
  ...
  while (start < n)
      ...
      for (qt = 0, j = i + 1; j <= n; j++)
	{
      ...
      if (j == n + 1)
	nrerror ("backtracking failed in ext loop");
      start = j + 1;
      backtrack (i, j);
    }
...
backtrack_qm1 (int i, int j)
{
  ...
  if (l > j)
    nrerror ("backtrack failed in qm1");
  backtrack (i, l);
}
...
backtrack (int i, int j)
{
  do
	...
	{
	  u1 = k - i - 1;
	  for (l = MAX1 (k + TURN + 1, j - 1 - MAXLOOP + u1); l < j; l++)
	    {
	      int type_2;
	      type_2 = ptype[iindx[k] - l];
	      if (type_2)
  ...
  /* backtrack in multi-loop */
  {
    ...
    if (k >= j)
      nrerror ("backtrack failed, can't find split index ");
	...
	if (k > j)
	  nrerror ("backtrack failed in qm");
