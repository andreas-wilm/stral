...
{ /* find roots of Hermite polynmials */
  ...
  for (ii = start; ii < n; ii++) {         /* search only upwards*/
    hroot[ii] = halfroot(hermite,n,hroot[ii-1]+EPSILON1, 1./n);
...
{ /* searches from the bound (startx) only in one direction
  ...
  while(fabs(fm) > EPSILON1) {        /* is root outside of our bracket?*/
...
void initname(long i)
{
  ...
  long j;
	...
	for (j=0;j<strlen(sdata[(int)i].ID);++j)
		nayme[i][j]=sdata[(int)i].ID[j];
} /* initname */
