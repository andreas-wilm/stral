...
#ifdef DEBUG
               if (i==0) {
                    fprintf(stderr, "DEBUG(%s:%s): hashing sequence of %s\n", __FILE__, __FUNCTION__, sdata[nseqs].ID);
               }
#endif		
               ++i;
          }
...
#ifdef DEBUG 			
          fprintf(stderr, "DEBUG(%s:%s): hashfilename=%s\n", __FILE__, __FUNCTION__, hashfilename);
#endif		
