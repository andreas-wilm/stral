...
int bionj(char *fname) {
	...
	if(input==NULL) {
		...
		while(input==NULL){
			...
			scanf("%s",Name_fich1);                     /* ask for the input-file */
			input=fopen(Name_fich1,"r");
		}
	}
	output = fopen("bionjout","w");
	...
	while(fscanf(input,"%d",&n) != EOF ) {
      ...
      *b=0;
      ...
      while (r > 3) {                        /* until r=3                */
			...
			strcat(chain1,":\n");
			sprintf(chain1+strlen(chain1),"%f\n",la);
